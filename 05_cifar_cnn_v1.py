...
valid_rate = 0.1
train_num = int(x_train.shape[0] * (1 - valid_rate))

x_valid = x_train[train_num:]
y_valid_one_hot = y_train_one_hot[train_num:]
x_train = x_train[:train_num]
y_train_one_hot = y_train_one_hot[:train_num]
...
with tf.Session() as sess:
    ...
    for i in range(10000):
        ...
        if i % 100 == 0:
            ...
            ls = loss.eval(feed_dict={x: batch[0], y: batch[1], keep_prob: 1.0})
            print("반복(Iteration): %d, 트레이닝 데이터 정확도: %f, 손실 함수(loss): %f" % (i, acc, ls))

            valid_batch = next_batch(128, x_valid, y_valid_one_hot.eval())
            acc = accuracy.eval(feed_dict={x: valid_batch[0], y: valid_batch[1], keep_prob: 1.0})
            ls = loss.eval(feed_dict={x: valid_batch[0], y: valid_batch[1], keep_prob: 1.0})
            print("반복(Iteration): %d, 밸리데이션 데이터 정확도: %f, 손실 함수(loss): %f" % (i, acc, ls))
