...
def build_CNN_classifier(x):
    ...
    b_output = tf.Variable(tf.constant(0.0, shape=[10]))
...
with tf.Session() as sess:
  ...
  for i in range(10000):
    ...
    if i % 100 == 0:
      ...
      print("반복(Iteration): %d, 트레이닝 데이터 정확도: %f" % (i, train_accuracy))

    # 옵티마이저를 실행해 파라미터를 한스텝 업데이트합니다.
    sess.run([train_step], feed_dict={x: batch_x, y: batch_y})

  # 학습이 끝나면 테스트 데이터에 대한 정확도를 출력합니다.
  print("테스트 데이터 정확도: %f" % accuracy.eval(feed_dict={x: x_test, y: y_test}))
